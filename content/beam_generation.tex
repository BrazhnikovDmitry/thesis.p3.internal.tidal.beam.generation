...
located and 
the weakest at a topographical gap having more gradual slopes \fignmlp[C3.fig:geo.map]{inset}. 
...
\fignmlp[C3.fig:BT]{A, B}. Dynamically though, as the impinging, ridge-emitted waves were 
...
integrated conversion rates \fignmlp[C3.fig:stand_wave]{D}. On average, each elemental wave field 
...
conversion \fignmlp[C3.fig:gen]{C}. The associated energy transfer processes are examined along a 
...
in \sqm{d10-15} a mode-1 wave originated at the Campbell Plateau and then was progressing across 
the ridge, 
...
\fignmlp[C3.fig:gen_2d]{F}, illustrating energy loss from the remote wave. As a consequence, the 
conversion time series on \fignml[C3.fig:gen_2d]{E, F} are in antiphase, while the 
...
\sqm{2014} the wave had not crossed the gap. This result is further illustrated by the directional 
decomposition and obtained wave phase distributions \fignmp{C3.fig:2d_phase}. It is seen as 
...
experiments and one added. The experiment \sqm{d35-40} manifested nonstationary 
character of the conversion process at the Macquarie Ridge as the energy transfer increased from 
1.6 GW to 1.9 GW in three week period \fignmlp[C3.fig:gen]{D}. The panels on 
...
\subsection{The remote waves and conversion levels}
\label{C3.sec:disc_conv}
The coupling of remote waves with local generation is addressed first as being a key process in 
setting magnitude of the energy transfer at the Macquarie Ridge. Obviously, amount of the remote 
wave energy reaching the western slopes of the ridge is an 
important quantity that depends on magnitude of the westward waves in the trough and transmission 
...
from statistical dependence of the terms and hence, non-zero covariance that has to be accounted in 
the sum. \footnote{For 
example, 
change in 
vertical velocity $w_{bt}$ also 
affects bottom baroclinic pressure producing 
non-zero covariance between the two. } Nevertheless, the obtained distribution on the western slope 
shows a pattern that is reminiscent to spatially inhomogeneous reflection of the eastern slope. 
That is, variability due to the phase lag concentrated near the gap and was smaller over the 
seamounts, while opposite held for amplitude of the bottom pressure. 
Physically, since in the gap the remote waves were large and strongly impacted local generation, 
both amplitude and phase of the bottom baroclinic pressure were disturbed. At the same time, 
on the 
steeper flanks of the seamounts the local generation was strong, while the remote waves were 
weak, so the coupling mainly affected the pressure's amplitude. Such difference in the resultant 
dynamical state is explained by a phasor diagram of superposition of the remote waves and locally 
generated internal tide.
amplitude . 
At  
is 
easily 
...
by plotting 
vector diagrams. The 
variability analysis demonstrates that on more gradual slopes remote waves perturb conversion 
stronger than on steep slopes. 
Such as at the Auckland Escarpment, all of variability is dictated by phase of the arriving 
eastward 
waves supporting discussion in Section \ref{C3.sec:3d_var}.\\
Additionally, there is about $10\%$ variations associated with the barotropic forcing which is 
mostly found in regions of the highly supercritical seamounts. Variable  steering of the barotropic 
tide could be named as a reason  \citep{holloway1999internal}. More likely though, distortions were 
introduced into vertically averaged component by strong shoaling of internal tide.\\


this feature also 
feature of the conversion 

calcula spatial average of 
the ea 
is used to 
emphasize 
spatial inhomogeneity 
in 
reflectance of the ridge's 
eastern 
slope. As the westward wave impinges the slope,  distribution of 
averaged criticality 

Transmission 


plays a key role. Transmission of the remote waves across 
...
distribution of conversion and its variation \fignmlp[C3.fig:gen]{C}. 
