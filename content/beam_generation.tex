...
whereas a gap separating the two to be smaller \fignmlp[C3.fig:geo.map]{inset}. However, 
...
conversion \fignmlp[C3.fig:gen]{C}. The associated wave mechanics is examined along a 
...
energy transfer can take. \fignml{C3.fig:gen_2d} is used to depict respective wave field 
developments on panels A-D throughout ebb stage of $M_2$ tidal cycle; E-F shows respective time 
series of the conversion in two designated locations on opposite slopes of the ridge. At 
ebb, on the ridge's west side \fignmlp[C3.fig:gen_2d]{A, C}, the downward barotropic current forced 
isopycnals to dip near bottom. At that moment in both experiments the energy was transferred from 
...
transfer became negative as, contrary to the forcing, isopycnals began to rise. The resultant 
elevated state was clearly seen at the moment of slack current \fignmlp[C3.fig:gen_2d]{B}. As the 
barotropic current reversed direction, the conversion changed sign again. Not surprisingly, the 
period-averaged energy transfer was weak \fignmlp[C3.fig:gen_2d]{E}. Experiment \sqm{d10-15} 
...
in \sqm{d10-15} a mode-1 wave originated at the Campbell Plateau was progressing across the ridge, 
it was evident by a characteristic vertical dipole pattern in the baroclinic pressure. The wave 
...
transfer. On the eastern side, the forcing was reversed, and the transfer was negative 
\fignmlp[C3.fig:gen_2d]{G, E} illustrating energy loss from the remote wave. In contrast, the 
mode-1 wave was not observed on the ridge slopes in \sqm{2014}. The eastern side exhibited positive 
conversion that led to internal wave radiation from the ridge crest \fignmlp[C3.fig:gen_2d]{A, 
dashed blue line}. Probably, the previously mentioned rising of the isopycnals on the western side  
in the middle of the ebb flow in \sqm{2014} resulted from propagation across the ridge of the 
...
wave phase distributions \fignmp{C3.fig:2d_phase}. The trough was occupied 
...
local generation. These results are consistent with the previous discussion. Additionally, near the 
ridge  
the decomposition showed the westward wave to have similar phase in both experiments. Then the 
further interaction with the local barotropic tide had to follow the same pathway with resultant 
conversion of the same sign. This is contrary to the presented results. The next section elaborates 
on the westward wave characteristics in order to explain the differences in the dynamics.\\
