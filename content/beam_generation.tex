...
\fignmp{C3.fig:beam}. The baroclinic tidal field appears as a complex pattern produced by multiple 
...
\fignmlp[C3.fig:gen]{b} and their spatial averages \fignmlp[C3.fig:gen]{c} a different perspective 
...
negative conversion (green shading, \fignml[C3.fig:gen]{b}). It is known \citep{Kelly2010a} 
...
interference creates a standing wave in Solander Trough \fignmp{C3.fig:stand_wave}. It is 
characterized by a node in horizontal kinetic energy with fluxes revolving in counterclockwise 
direction \fignmp{C3.fig:stand_wave} as it should be in the Southern Hemisphere. By the directional 
method (appendix \ref{C3.app:A1}) the standing wave is separated into elemental east-west oriented 
components \fignmlp[C3.fig:stand_wave]{b, c}. On the leeward side of the Macquarie Ridge generation 
...
trough 
and comparing with spatially integrated conversion rates \fignmlp[C3.fig:stand_wave]{d} highlights 
...
distribution. To illustrate this further let detail dynamics along a transect crossing the sill 
(horizontal line on \fignml[C3.fig:gen]{b}) in simulations with diametrically different generation 
regimes (\sqm{2014} and \sqm{d10-15, 2015}, \fignml[C3.fig:gen]{c}).\\
...
\fignmlp[C3.fig:gen_2d]{a, 
b} 
during ebb tide on the tideward side (solid black line, $164.2^{\circ}$) there is vertically net 
energy 
conversion into the baroclinic field \fignmlp[C3.fig:gen_2d]{e}. But note along the bottom an 
...
\fignml[C3.fig:gen_2d]{e} the net conversion becomes negative, but then flips sign as the 
tidal currents reverse direction \fignmlp[C3.fig:gen_2d]{b}. Averaged over tidal period the 
transfer is positive, i.e. the surface tide losses energy, but weak compare to \sqm{d10-15} 
simulation 
\fignmlp[C3.fig:gen_2d]{c, d}. In the latter there is an enhancement of generation at the sill due 
to presence of non-local internal tide. Its propagation from Solander Trough ($165^{\circ}$) and 
over the sill is the major difference between the two experiments. In setting of \sqm{d10-15} 
...
vertical 
component of the barotropic flow has the opposite direction. For \sqm{d10-15} the traveling 
mode-1 is 
losing energy as it is displacing isopycnals contrary to the forcing. While in \sqm{2014} there is 
reflection seen as a change in phase progression from upward-westward to downward-eastward in an 
internal wave in the trough \fignmlp[C3.fig:gen_2d]{a, b}. The reflection coincides with the 
upward barotropic flow that results in higher production of the baroclinic tide. And the  
conversions highlights this by having opposite signs on the leeward side 
\fignmlp[C3.fig:gen_2d]{f}. Overall, the simulations demonstrate two distinct cases of 
coupling between barotropic and baroclinic modes on a sloping bottom. In the first case there is 
coupling of 
barotropic tide 
during reflection and second - coupling 
of bt tide while remote wave shoals. This can be easily illustrated by considering phases.

These points are further explored on \fignmlp[C3.fig:gen_2d]{g} 
where phase distribution of the mode-1 along the section is given..... The total signal shows a 
region 
of almost constant phase change eastward of $165^{\circ}$ that corresponds to 
concentration of the node in HKE (\fignml[C3.fig:stand_wave]{a}, dashed line marks the considered 
