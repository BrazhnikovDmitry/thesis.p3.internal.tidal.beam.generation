...
as seen in a summary table 2. Further, plan-view comparison \fignmp{C3.fig:cmp_sat_nexp} provides 
...
performed experiments are by no means sufficient to derive a statistically meaningful mean. These 
...
considered. At first, in 2013 HYCOM atmospheric forcing was modified with consequent surface 
winds having smoother curl. Thus, different behavior of the upper ocean is expected, but timing of 
the increase does not coincide with the model forcing switchover (given by a mark). At second, 
large-scale oceanographic implications involve repositioning of the Subtropical Frontal Zone (STFZ) 
that separates subtropical from subantarctic water masses. Averages of sea surface salinity used as 
definition of the STFZ showed a 1$^{\circ}$ equatorward displacement of the southern boundary. Such 
...
For any reason of the jump, different intensity of mesoscale currents will inevitably drive 
varying amount of refraction. On \fignm{C3.fig:KE_beam} this is depicted by diamonds 
representing the beam displacement relative to the satellite-derived beam that were all southward. 
Over the first period with weaker currents, the beam is found in northward position and closer to 
the satellite altimeter. And after 2013 austrial summer intensification the beam is located 
poleward. Thus, the performed numerical experiments are biased towards poleward location due to the 
considered sample of the oceanic conditions. This finding further opens a possibility for a bias in 
the satellite investigation \citep{zhao2018satellite} that covered 1993-2012 and application of the 
internal tides for deducing oceanic changes.\\

The TBEAM field program took place in January-March 2015 \fignmlp[C3.fig:KE_beam]{inset}. The field 
period simulation diagnosed southward beam position with $\pm 15~km$ shifts and overall baroclinic 
tide intensification \fignmlp[C3.fig:gen_var_beam]. These prepositions will be assessed against the 
in-situ observations \citep{waterhouse2018observations}. The TBEAM program comprised a stationary, 
moored measurements over period of 49 days (A1 mooring) 
...
A1 mooring measured a convoluted, highly variable history of the baroclinic tide at a single 
location, and the LADCP stations gave a perspective on spatial extent and structure of the beam. 
...
uncertainties and analysis is to be found in \citep{waterhouse2018observations}. Part of the  
investigation included fitting a plane wave providing an estimate on the bulk beam parameters 
(table). All three approaches, altimetry, in-situ and numerical modeling, compare well and combined 
provide the averaged internal tide characteristics. Yet the differences are valuable in order to 
gain further insights. So let consider the total field for the gravest mode 
\fignml[C3.fig:TBEAM_sp]{Panel A}. There is a large contrast between the in-situ observations and 
the ROMS experiment \fignmlp[C3.fig:TBEAM_sp]{Panel a, pink and red arrows}. At the stations 
closest to ETP (F2-F5 and A1) the simulated energy fluxes are oriented towards south. This 
disparity comes from interference between the beam component (blue arrows) and oppositely traveling 
reflected waves (green arrows). If the beam component is considered only, then the comparison is 
more favorable and was emphasized by the table. In fact, at location of A1 the simulated beam 
coincides with the mean energy flux at the mooring. Overall, this leads to conclusion that the 
simulated total field has different characteristics of interference than it was observed in-situ. 
This point is emphasized by examining a numerical experiment where the beam was closest to the 
satellite-derived one \fignmlp[C3.fig:TBEAM_sp]{Panel b}. Now the total field (pink arrows) 
corresponds to the observations much better at F2-F5, A1. Nevertheless, at other stations mismatch 
is still large. This is not surprising because of superposition with the waves emerging from South 
Tasman Rise. If that signal is removed, the comparison is better (not shown). Hence, the actual 
observations are less influenced by internal tidal swell.\\
\fignml[C3.fig:TBEAM_sp]{Panel a and b} emphasize that the total wave field is highly variable 
because of changeable beam and reflection. This is then augmented by a partial standing wave and 
spatial distribution of HKE and APE. On average \fignmlp[C3.fig:TBEAM_KE2PE]{Panel a} the TBEAM 
stations are found in different locations characterized both by an excess of kinetic or 
potential energies. Similar trends are observed in the in-situ observations, but direct comparison 
for LADCP stations is not feasible because of near-inertial wave energy contamination. On the other 
hand, the moored observations at A1 provides time-series of the ratio HKE to APE 
\fignmlp[C3.fig:TBEAM_KE2PE]{Panel d}. In the first period of observation APE was much larger than 
kinetic part, this is thought as a node was located nearby. In the later period the ratio was 
tending towards a plane wave value, and hence, the node magnitude was lessen either because of 
change in position or diminish of interference phenomena. The numerical simulation shows less 
magnitude of variability and different time history. Nevertheless, it is used to exemplify 
standing wave behavior \fignmlp[C3.fig:TBEAM_KE2PE]{Panel b, c}. Two regimes are presented when the 
anti-node is relatively weak and located away from A1 and the opposite situation when interference 
is strongly developed. It is seen that such shifts are primarily driven by proportion and spatial 
distribution of the reflected energy. Further, note that even variability concerns the anti-node 
strength and position, potential energy is more stationary, i.e. its relative change is smaller 
than one in kinetic energy. This is simply because any change in relatively low levels of kinetic 
energy will be more noticeable. This can be illustrated in a one-dimensional problem 
\citep{martini2007diagnosing} where 
\begin{align}
E_{KE} \sim (1 - r)^2 + 4r \sin^2{k x},~E_{PE} \sim (1 - r)^2 + 4r \cos^2{k x}
\end{align}
taking differential will lead to
\begin{align}
\frac{\delta E_{KE}}{E_{KE}}\big|_{kx = 0} = \delta r \frac{2}{1 - r},~\frac{\delta 
	E_{PE}}{E_{PE}}\big|_{kx = 0} = \delta r \frac{2}{1 + r}
\end{align}
where $\delta r$ - a small perturbation in amplitude ratio of two interfering waves. Now at the 
anti-node locations ($kx = 0$) any changes in wave content will be amplified in the kinetic energy. 
The amplification factor becomes increasingly large as $r \rightarrow 1$ and a pure standing wave 
develops. In the numerical experiments A1 showed ... \\
During the TBEAM observation measurements of potential energy were more stable than kinetic energy. 
This lends additional confidence that the A1 mooring was located close to anti-node and variability 
in the energy flux was caused by interference pattern adjustments. These inferences can be further 
tested by spectral decomposing. Since it is a point-wise observation the used method previously can 
not be directly applied. But if it is assumed that interfering waves do not have any phase lags or 
that cross-terms in energy quantities are negligible, then non-deterministic decomposition can be 
applied. The actual signals will not be back-synthesized and only some energy characteristics could 
be deduced. The assumption is fulfilled in position between node and anti-node where two 
interfering waves are in quadrature. So A1 location makes it possible to deduce parameters of the 
wave components taken part in interference.\\
The details of the non-deterministic decomposition are given in Appendix A.2 and an example 
is shown on \fignm{C3.fig:ex_spectra}. The fit can obtain only 5 Fourier circular components which 
leads to low angular resolution, only waves separated by $180^{\circ}$ can be separated. In more 
complicated wave field this might lead to some wrongly identified direction. This is seen in 
comparison between application of the pointwise method and the decomposition based on antenna in 
case of the numerical experiment. Here incident direction was wrongly identified. To further assess 
applicability, numerical simulation results from a half wavelength window surrounding A1 location 
were separated by ratio in HKE and APE. Then the point-wise decomposition was applied and compared 
to the antenna analysis \fignmp{C3.fig:test_spectra}. Clearly depending on position importance of 
phase lags and hence, cross-terms either decrease or increase leading to biases as the ratio in HKE 
to APE is increasing. The worst case is for when measurements are made near a node. But 
surprisingly, the best result is found for locations close to anti-node when the ratio is less than 
1. Here energy of the incident wave was identified correctly as well as its bearing. The reflected 
portion on the other hand was underestimated, and its travel direction is absolutely incorrect. 
Overall, the point-wise decomposition provides a conservative estimate of the incident wave field 
and a qualitative picture of the reflected wave.\\
Now the point-wise decomposition is applied to the TBEAM observations \fignmp{fig_22_A1_comp.png}, 
for comparison the numerical results are added. The amount of incident energy closely follows 
total signal and there were two periods of intensification. In the numerical simulations this 
happened because of growth in the baroclinic tide production at the Macquarie Ridge. There is a 
remarkable resemblance with the field observations \fignmlp[C3.fig:dcmp_A1]{Panels a, c}. So 
that non-stationary generation can be considered as a possible reason for change in magnitude of 
flux observed in the TBEAM time series. Secondary, the bearing of the incident wave also varied. 
This probably was produced as a result of refraction by an anticyclonic eddy that was present 
during the field period. The ROMS model had a moderate skill in simulating mesoscale dynamics. 
Though eddy was situated a half degree south $\sim 70~km$ in comparison either to AVISO or HYCOM, 
...
energy flux turned southward \fignmlp[C3.fig:dcmp_A1]{Panel d, dots}. Similar pattern of change is 
observed in the total flux of the TBEAM (gray line) and point-wise decomposition (black line). Yet 
if the total wave field in the numerical experiment is considered \fignmlp[C3.fig:dcmp_A1]{Panel 
b}, then resemblance is poor. It suggests that reflected signals in reality are weaker and 
modulations due to interference are less pronounced. This is envisaged from 
\fignml[C3.fig:dcmp_A1]{Panel e} presenting reflected portion. The point-wise decomposition 
clearly overestimates the magnitude by about 3-4 times as it was presented before. Thence, the 
reflected wave was at maximum $0.5~kW/m$ during the TBEAM observations. This is much smaller in 
comparison to the numerical simulations. The reason for this mismatch comes from scrutiny of 
sources for the reflected wave. These are reflected waves from Tasmania, reflected waves from ETP 
and the Cascade Seamount and local generation at the seamount. The latter constitutes the major 
portion. Dynamics of generation at this location is similar to the Macquarie Ridge, i.e. phasing of 
the incident beam relative to local barotropic tide will lead to either production of the internal 
tide or destruction. And as the region is highly dynamical both in terms of internal tide dynamics 
as many components of the wavefield interact with the seamount as well as local conditions of the 
mesoscale. For example, in all the experiments with mesoscale currents there is an overall 
generation at the seamount, but in the 'uniform' experiment the internal tides are destroyed. So it 
seems likely that the ROMS experiments were not able to predict this part correctly. Nevertheless, 
the pointwise decomposition seems to capture pattern of temporal variability in the reflected wave 
correctly. More precisely, during January 2015 there was a considerable fraction of the reflected 
wave present. And as a result the total flux was leaning towards West, the ratio KE/PE was below 1. 
Similar notions can be applied to the burst of reflected energy during yearday 30-35 as the 
ratio dipped \fignmp{C3.fig:TBEAM_KE2PE} as well as magnitude of the total flux. In the 
later part of the observational period reflected wave is minuscule part of the signal and 
...
nonstationarity in the observations, so that flux magnitudes and direction can largely vary on 
time scales compared to semidiurnal period. Energy transfer between mesoscale and tidal field was 
not discussed here, but seems to be of negligible importance at least during the considered period. 
For the purposes of providing boundary conditions for the reflection processes in Tasmania, 
the presented variability will be inevitably exaggerated downstream, so that reflection process 
is not a simplistic back-scatter of plane wave.
